...
>
> **v0.8.0-dev: a maximally vague `object_name` no longer returns a false full-frame match.**
> `detect_objects("object")` on a blank canvas previously returned a box spanning nearly the
> whole frame — Florence-2's grounding head has no explicit "nothing here" output for a query
> with nothing distinctive to point to, and falls back to the whole image instead. Only the
> exact vague vocabulary itself ("object", "thing", "item", "stuff", "something", "shape",
> "entity", and their plurals — see `query_policy.py`) now suppresses that fallback box; a
> specific noun that genuinely fills the frame (a close-up of wood, asked for "wood") is
> returned exactly as before, since the guard is triggered by the query's wording, not by how
> much of the frame a box covers.
...
> **v0.8.0-dev: the same generic-noun/full-frame guard as `detect_objects`, extended to this tool's plain
> (non-`clip_art`) path.** Grounding DINO has the same fallback-to-the-whole-frame failure mode on a
> maximally vague name — measured: `count_objects("object")` on `tests/detect_blank_canvas.png` previously
> counted a box spanning nearly the whole frame (score 0.46) as one real instance. Only the exact generic
> vocabulary (`query_policy.is_generic_query`) triggers it, and only the near-full-frame entries it finds —
> a genuine small detection returned alongside a spurious full-frame one for the same generic query is kept.
> `clip_art=true` already had the equivalent guard through Florence-2's own `exclude_full_frame`.
...
> **v0.8.0-dev: a maximally vague name is guarded the same way `detect_objects`/`count_objects` are.**
> A name like `"object"` or `"thing"` with nothing distinctive to point to can come back as one box
> spanning nearly the whole frame rather than nothing — measured on `tests/detect_blank_canvas.png`,
> `objects=["object"]` previously reported that full-frame box as the "located" match for `"object"`.
> Now filtered before the best-scoring-box selection, so it comes back as not found (`objects: []`,
> the existing "none of the requested objects were found" note) instead. Only the exact generic
> vocabulary is affected; a specific name is located exactly as before.
...
*New in v0.8.0*: This tool now surfaces `photographic_aesthetic` and uses domain routing to determine if that score is meaningful (`photographic_aesthetic_applicable`). If the optional `[iqa]` extra is installed, it also executes a MUSIQ model for `technical_quality`. If a reasoner is configured (`--reasoner-provider ollama`), it synthesizes an `artistic_judgment`.
