...
async def run_battery():
    server_params = StdioServerParameters(command="uv", args=["run", "fusion-vision-mcp"])
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            out_cap = results["caption_flower"]["output"].get(
                "caption", results["caption_flower"]["output"].get("text", "")
            )
            ...
            print("[OCR] Defect -> (Output logged)")

            # Test 3: Object Counting (flower)
            t0 = time.time()
            res = await session.call_tool(
                "count_objects", arguments={"src": FIXTURES["flower"], "object_name": "petal"}
            )
            t1 = time.time()
            results["count_flower"] = {"time": t1 - t0, "output": parse_res(res.content[0].text)}
            print(
                f"[Count] Flower petals -> {results['count_flower']['output'].get('count')} (Ambiguous: {results['count_flower']['output'].get('semantic_ambiguity')})"
            )
            ...
            res = await session.call_tool(
                "query_image",
                arguments={"src": FIXTURES["defect"], "question": "What's wrong?", "structured_analysis": True},
            )
            t1 = time.time()
            results["query_defect"] = {"time": t1 - t0, "output": parse_res(res.content[0].text)}
            print(
                f"[Structured Analysis] Defect -> anomalies: {len(results['query_defect']['output'].get('evidence', {}).get('structured_analysis', {}).get('anomalies', []))}"
            )
